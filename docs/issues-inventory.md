...
| PHB-INT-003 | `docs/migration-notes.md` still carries the `sc-observability 1.2` cutover section without an explicit owning cleanup sprint after Phase B close. | Planned | `B13` | Keeps the remaining `sc-observability 1.2` migration-note alignment with the observability cleanup line instead of leaving it untracked. |
