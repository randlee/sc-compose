...
distribution can resolve every Python utility transitively used by the CI lint
...
originating target sprint. For each routed finding, write a note keyed by
`<rule-id> + <file-path>` under `reports/inputs/lint/lint-ci/`, naming the
atomic owner sprint and known fix branch/PR. Keep CI workflow, constants, and
...
L.15 cannot close until fixes are QA-approved, merged, and rerun.
...
- When L.1 records all transitive Python utilities as resolvable, the positive
  fixture produces a pass report with command lint.ci and a retained raw JSON
