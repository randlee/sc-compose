...
phase: L
status: complete
branch: fix/l-12-stale-rebase-comp3
worktree: ../sc-compose-worktrees/sprint/l-12-clippy-xwin
target: integrate/phase-l
---

# Sprint L.12 — Windows Cross-Target Clippy

## Goal

Integrate the single sc-lint command target clippy xwin through the shared L.2 runner,
...
After L.2 is merged, L.12 may execute in parallel with L.3, L.4, L.5, L.6,
L.7, L.8, L.9, L.10, L.11, L.13, L.14, L.15, and L.16. It does not run in
parallel with prerequisite L.1/L.2 or final L.17.

## Exact Targets

- .sc/sc-lint/targets/clippy-xwin.toml
- tests/fixtures/sc-lint/clippy-xwin/
- crates/sc-compose/tests/sc_lint_clippy_xwin.rs
- reports/inputs/lint/clippy-xwin/

## Deliverables

- One declarative target descriptor mapping clippy xwin to the canonical sc-compose
...
- Run the cargo xwin clippy path when available; when unavailable, preserve the explicit capability error and never silently report pass.
...
- No representative Python helper exists for clippy xwin in sc-lint 0.4.0.
...
- The positive fixture produces a pass report with command clippy.xwin and a retained
  raw JSON artifact.
...
- The target can be run through the standard just command clippy xwin.
...
- just clippy xwin
- cargo test -p sc-compose --test sc_lint_clippy_xwin
- cargo fmt --all --check
- git diff --check
