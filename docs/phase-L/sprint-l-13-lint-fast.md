...
distribution can resolve every Python utility transitively used by the fast
...
After L.2 is merged, L.13 may execute in parallel with L.3, L.4, L.5, L.6,
L.7, L.8, L.9, L.10, L.11, L.12, L.14, L.15, and L.16. It does not run in
...
duplicate fix. For each routed finding, write a note keyed by `<rule-id> +
<file-path>` under `reports/inputs/lint/lint-fast/`, naming the atomic owner
sprint and known fix branch/PR. Keep profile changes, constant strings, and
length refactors separate. Send each worktree or routing note and its commit
to team-lead for PR creation; team-lead sends the PR to quality-mgr for QA.
L.13 cannot close until required fixes are QA-approved, merged, and rerun.
...
- When L.1 records all transitive Python utilities as resolvable, the positive
  fixture produces a pass report with command lint.fast and a retained raw JSON
  artifact. When L.1 records any utility as unresolvable, the positive-path
