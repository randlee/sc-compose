# crates/sc-compose — Repowise Code Health

**Version:** release-candidate-v1.6.1-10-gb763d2ff | **Commit:** b763d2ff | **Generated:** 2026-08-31
**Analyzed:** 207 files | **Biomarker findings:** 1129 | **Data:** repowise health + dead-code + refactoring-targets
...
|---|---|---|
| duplicated_assertion_block | 336 | Repeated assertion patterns — test helper opportunity |
| error_handling | 185 | Error handling gaps or inconsistencies |
| dry_violation | 114 | DRY violations — opportunities to extract shared code |
| prior_defect | 110 | Files with bug-fix history — strong defect predictor |
| co_change_scatter | 58 | Files that change together — high coupling |
| hot_path_sync_io | 50 | Sync I/O on hot paths — should be async |
| change_entropy | 40 |  |
| io_in_loop | 37 |  |
| complex_method | 36 |  |
| large_method | 30 |  |
| primitive_obsession | 30 |  |
| churn_risk | 28 | High recent change frequency — churn-driven risk |
| nested_complexity | 22 |  |
| function_hotspot | 21 |  |
| low_cohesion | 15 |  |
| untested_hotspot | 7 | Depended-upon files with no paired test coverage |
...
### [336] duplicated_assertion_block
...
- **high** `crates/sc-compose/src/cli/schema.rs` `(top-level)`: co-changes with 25 distinct files — editing this file tends to ripple across the codebase (shotgun surgery)
...
3. **Extract duplication** — 336 duplicated assertion blocks; shared test-helper modules would remove the bulk.
4. **Audit sync I/O on hot paths** — 50 findings; either make async or document the intentional sync boundary.
