...
**Analyzed:** 203 files | **Biomarker findings:** 1258 | **Data:** repowise health + dead-code + refactoring-targets
...
|---|---|---|
| duplicated_assertion_block | 333 | Repeated assertion patterns — test helper opportunity |
| error_handling | 181 | Error handling gaps or inconsistencies |
| hot_path_sync_io | 175 | Sync I/O on hot paths — should be async |
| dry_violation | 114 | DRY violations — opportunities to extract shared code |
| prior_defect | 105 | Files with bug-fix history — strong defect predictor |
| co_change_scatter | 63 | Files that change together — high coupling |
| change_entropy | 40 |  |
| complex_method | 37 |  |
| io_in_loop | 36 |  |
| large_method | 33 |  |
| primitive_obsession | 30 |  |
| churn_risk | 29 | High recent change frequency — churn-driven risk |
| nested_complexity | 24 |  |
| function_hotspot | 24 |  |
| low_cohesion | 14 |  |
| untested_hotspot | 9 | Depended-upon files with no paired test coverage |
...
### [333] duplicated_assertion_block
...
3. **Extract duplication** — 333 duplicated assertion blocks; shared test-helper modules would remove the bulk.
4. **Audit sync I/O on hot paths** — 175 findings; either make async or document the intentional sync boundary.
