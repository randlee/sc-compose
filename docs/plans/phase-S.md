...
| S-T5 | `crates/sc-compose/src/commands/compose.rs`; `run_checked_validate` lines 182–253 mixes four concerns | Extract private checked-report construction and output/exit handling, with fixtures that freeze JSON and text output. | Rule 2: safe in CLI; `sc-composer` remains the composition/check authority. | S.2 |
| S-T6 | `crates/sc-compose/src/cli/capability.rs`; `command_wants_json` CCN 22 | Decompose nested subcommand checks into private capability helpers and exhaustive table-driven tests. | Rule 2: safe in CLI only; no clap/public-argument change. | S.2 |
| S-T7 | `crates/sc-compose/tests/repo_boundaries.rs`; one CCN 18 test combines discovery and policy | Split discovery from named invariant-family assertions and add focused regression fixtures. | Rules 1–13: safe because it strengthens enforcement only; it may not remove or relax a prohibited dependency assertion. | S.3 |
| S-T8 | `crates/sc-composer-beads/src/runner.rs`; three recent defect fixes and process-containment state at lines 78–137 | Partition private capture-state handling from child containment/status collection; add Unix and Windows contractual tests without changing request/receipt contracts. | Rule 11: safe only with existing `process-wrap` and host-neutral runner boundaries; no CLI, Beads source/database, foreign adapter, or ATM dependency. | S.4 |

Additional coverage accompanies these refactors: diagnostics module contract tests
for the high-dependent re-export facade (`crates/sc-composer/src/diagnostics.rs`)
and path serialization/normalization tests for the high-dependent CLI helper
(`crates/sc-compose/src/path_utils.rs`). They are reliability coverage, not
pretext for a public API rewrite.
...
| [S.2](../phase-S/sprint-s-2-cli-contract-seams.md) | Split CLI lint, checked-validation, and JSON-capability concerns. | None. | May implement in parallel with S.1, but merges forward from the latest `integrate/phase-s`. |
| [S.3](../phase-S/sprint-s-3-boundary-and-path-guardrails.md) | Make boundary and path/diagnostic guardrails independently testable. | None. | May implement in parallel; rebase/merge-forward after S.1/S.2 before phase closeout. |
| [S.4](../phase-S/sprint-s-4-beads-runner-reliability.md) | Isolate the cross-platform output-capture lifecycle and prove containment. | S.3 test-guardrail conventions. | Last; high-risk runner work needs the strengthened regression baseline. |
...
- [ ] Every S-T1 through S-T8 refactor preserves its existing public behavior
