...
2. Validate the manifest and candidate tag, then run `Release Preflight` with
   ...
   prior root release. Pass that comma-separated value to the Release
   Preflight `already_published_channels` workflow input. Do not infer it from
   a registry lookup or include a channel without that prior passed evidence;
   leave the input empty when no channel qualifies.
