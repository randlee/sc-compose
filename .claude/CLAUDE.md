...
## Codebase Intelligence for repowise-audit-3 (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-08 (commit 5fc2f38)
...
- `crates/sc-compose/tests/cli.rs` — 43 bug fixes, last fix 10 days ago (bug magnet); 61 commits/90d
- `crates/sc-compose/src/main.rs` — 30 bug fixes, last fix 3 weeks ago (bug magnet); 29 commits/90d
- `crates/sc-compose/tests/json_cli.rs` — 27 bug fixes, last fix 3 weeks ago (bug magnet); 42 commits/90d
- `crates/sc-compose/src/reporting/render_many.rs` — 20 bug fixes, last fix 2 days ago (bug magnet); 24 commits/90d
- `crates/sc-compose/src/var_file.rs` — 15 bug fixes, last fix 2 days ago (bug magnet); 25 commits/90d

### Code health
Three co-equal signals: defect risk 7.78/10 avg, hotspot health 6.17/10 (stable), worst `crates/sc-composer/src/extract/xml.rs` at 1.0/10 · maintainability 9.27/10 · performance risk 120 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `docs/phase-D/sprint-d-4-template-init-verify.md` — change entropy — impact −2.3
- `docs/phase-D/sprint-d-2-py-bindings.md` — churn risk — impact −2.2
- `docs/requirements.md` — untested hotspot — impact −2.0
- `docs/architecture.md` — untested hotspot — impact −2.0
- `crates/sc-composer/tests/integration.rs` — prior defect — impact −2.0
