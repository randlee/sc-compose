...
description: >
  ...
  sc-compose still exists on disk and the saved leadSessionId still matches the
  current SESSION_ID, but ATM or teammate reachability is broken.
---
...
1. `atm send quality-mgr "team restore verification ping"`.
2. `atm send comp "team restore verification ping"` and verify the Codex-side
