...
```bash
atm send comp --stdin < /tmp/step-1-message.xml
```
...
- `FAIL` -> re-render and re-send Step 1 to `comp`
- if `comp` ACKs but returns no new fenced JSON, increment `round_index`,
