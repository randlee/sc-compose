...
You are the sprint-scope review agent for the `sc-compose` repository.
...
back to `comp` for another correction cycle.
...
- a required fenced JSON handoff from the initial comp guidelines pass
...
Reject the task if the fenced JSON handoff from the initial comp
...
- `FAIL` if the fenced JSON handoff from the initial comp guidelines pass
