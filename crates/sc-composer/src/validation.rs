...
fn collect_identifiers(
    ...
    for candidate in mask_quoted_literals(expression).split(|character: char| {
        ...
        let root = candidate.split('.').next().unwrap_or(candidate);
        if bound_names.contains(root) || is_loop_context_name(candidate, scopes) {
            continue;
        }
...
fn is_loop_context_name(candidate: &str, scopes: &[LoopScope]) -> bool {
    !scopes.is_empty() && LOOP_CONTEXT_NAMES.contains(&candidate)
}
